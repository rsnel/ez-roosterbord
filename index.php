...
<?
		...
		while ($row[UUR] == $i) {
			...
			$info = array();
			if (isset($row['flex_info'])) {
				add_flex($info, $row['flex_info'], $row[VAKKEN]);
			} else {
...
function make_link($target, $text = NULL, $day = NULL) {
	...
	if (!$day || $day == $_GET['dy'] || $_GET['dy'] == '*') return '<a data-transition="flip" href="?'.(isset($_GET['m'])?'m&amp;':'').'q='.urlencode($target).$link_tail.($text?$text:htmlenc($target)).'</a>';
...
function cleanup_flex(&$row) {
	if (preg_match('/^(flex[1-4])(,flex[2-4])*$/', $row[VAKKEN])) {
		...
		$row[VAKKEN] = 'flex';
		$row[LESGROEPEN] = '';
	} else if (preg_match('/^(study[1-4])(,study[2-4])*$/', $row[VAKKEN])) {
		$row['flex_info'] = $row[LESGROEPEN];
		$row[VAKKEN] = 'study';
		$row[LESGROEPEN] = '';
	}
	if (preg_match('/^(flex[1-4])(,flex[2-4])*$/', $row[VAKKEN2])) {
		$row[VAKKEN2] = 'flex';
		$row[LESGROEPEN2] = '';
	} else if (preg_match('/^(study[1-4])(,study[2-4])*$/', $row[VAKKEN2])) {
		$row[VAKKEN2] = 'study';
		$row[LESGROEPEN2] = '';
	}
}
...
function add_flex(&$info, $lesgroepen, $vak) {
	$info[] = make_link($lesgroepen, $vak);
}
			...
			if ($row[WIJZ_ID]) { // deze les is: extra/nieuw, lokaalreservering, (fake)verplaatstvan of gewijzigd
				...
				if (!$row[DAG2] || (!$row[VIS2] && $row[VIS])) { // bij deze les hoort geen oude les, dus: extra, reservering of fakeverplaatstvan
					if ($row[VAKKEN] == 'lok') {
						...
						if (isset(config('VERBERG_EXTRA_LESSEN_MET_DEZE_VAKKEN')[$row[VAKKEN]])) {
							$row = $result->fetchRow();
							continue;
						}
						$extra = ' extra';
